...
declare global {
    interface CanvasRenderingContext2D {
        setRange: (min: number, max: number) => CanvasInfo
...
interface CanvasInfo {
    ...
    clearCanvas: (clearColor: string) => void
}
...
export function createCanvas(width: number, height: number, canvasId: string = 'mainCanvas'): CanvasRenderingContext2D {
    ...
    // installValuesCheck(ctx.canvas)

    ctx.setRange = setCanvasRange.bind(ctx)
    ctx.clear = clear.bind(ctx)
    ctx.draw = (...params) => draw(ctx, ...params)
...
export function setCanvasRange(
    ...
    // Calculate the longest diagonal (span)
    const calculateDistance = (x: number, y: number) => Math.sqrt(x * x + y * y)
    const span = calculateDistance(xRange[1] - xRange[0], yRange[1] - yRange[0])
